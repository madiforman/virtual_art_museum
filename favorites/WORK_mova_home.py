...
if st.session_state.favorites:
    favorites_data = pd.DataFrame(st.session_state.favorites)
    for idx, favorite in favorites_data.iterrows():
        col1, col2 = st.columns([3, 1])
        with col1:
            st.image(favorite['image'], caption=favorite['title'])
        with col2:
            if st.button(f"❌ Remove from Favorites", key=f"remove_{idx}"):
